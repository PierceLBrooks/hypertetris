...
ZPiece::ZPiece() : GenPiece(5)
{
  ...
  int **newpos = new int*[5];
  
  for (int i = 0; i < 5; i++)
    newpos[i] = new int[4];

  for (int i = 0; i < 4; i++) // cube 0
    newpos[0][i] = dudeman[i] + ((i == Y) ? 1 : ((i == X) ? -1 : 0));

  for (int i = 0; i < 4; i++) // cube 1
    newpos[1][i] = dudeman[i] + ((i == Y) ? 1 : 0);

  for (int i = 0; i < 4; i++) // cube 2
    newpos[2][i] = dudeman[i];

  for (int i = 0; i < 4; i++) // cube 3
    newpos[3][i] = dudeman[i] + ((i == Y) ? -1 : 0);

  for (int i = 0; i < 4; i++) // cube 4
    newpos[4][i] = dudeman[i] + ((i == Y) ? -1 : ((i == X) ? 1 : 0));
...
*/
  ...
  for (int i = 0; i < 5; i++) {
    ...
    //cubes[i]->SetColor(0x1ab23c);
    cubes[i]->SetColor(0x0066ff);
  }
